...
impl Keys {
    ...
    /// Get [PublicKey] by [Amount]
    pub fn get_public_key(&self, amount: &Amount) -> Option<PublicKey> {
        self.0.get(amount).copied()
    }
}

impl JsonUtil for Keys {
    type Err = Error;
}
